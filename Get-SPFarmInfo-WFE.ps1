...
function main
{
    ...
    if($PatchInfo)
    {
        ...
        if ($choice -ne 0) {
            #No
            Write-LogEntry "Disabling PatchInfo Gathering"
    ...
    {
        Write-LogEntry "Site $siteUrl Not Found"
    ...
    Write-LogEntry "Generating Output file"
...
#        $fileName = $fileName + ".html"
    ...
    {
        Write-LogEntry "Generating Hashes for Servernames and Addresses"
...
#>
    Write-LogEntry ("`n`nScript complete, review the output file at `"{0}`"" -f $fileName)
